            ...
            for csr in region.obj:
                ...
                if hasattr(csr, "fields"):
                    for field in csr.fields.fields:
                        ...
                        if with_access_functions:
                            r += "static inline uint32_t "+name+"_"+csr.name+"_"+field.name+"_extract(uint32_t oldword) {\n"
                            r += "\tuint32_t mask = ((1 << "+size+")-1);\n"
                            r += "\treturn ( (oldword >> "+offset+") & mask );\n}\n"
                            r += "static inline uint32_t "+name+"_"+csr.name+"_"+field.name+"_inject(uint32_t oldword, uint32_t plain_value) {\n"
                            r += "\tuint32_t mask = ((1 << "+size+")-1);\n"
                            r += "\treturn (oldword & (~(mask << "+offset+"))) | (mask & plain_value)<< "+offset+" ;\n}\n"
                            if (not isinstance(csr, CSRStatus)):
                                r += "static inline void "+name+"_"+csr.name+"_"+field.name+"_write(uint32_t plain_value) {\n"
                                r += "\tuint32_t oldword = "+name+"_"+csr.name+"_read();\n"
                                r += "\tuint32_t newword = "+name+"_"+csr.name+"_"+field.name+"_inject(oldword, plain_value);\n"
                                r += "\t"+name+"_"+csr.name+"_write(newword);\n"
                                r += "}\n"
